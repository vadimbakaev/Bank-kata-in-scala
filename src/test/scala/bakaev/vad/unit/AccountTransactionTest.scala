...
class AccountTransactionTest extends FlatSpec with Matchers with MockitoSugar {
  ...
  it should "return new account Transaction with new Transaction on deposit" in {
    ...
    when(seq :+ Transaction(Amount(10), LocalDate.MIN)).thenReturn(Seq(Transaction(Amount(10), LocalDate.MIN)))
    ...
    verify(seq) :+ Transaction(Amount(10), LocalDate.MIN)
  }
  ...
  it should "return new account TransactionStorage with new Transaction and negative Amount on withdraw" in {
    val seq = mock[Seq[Transaction]]
    when(seq :+ Transaction(Amount(-1000), LocalDate.MIN)).thenReturn(Seq(Transaction(Amount(-1000), LocalDate.MIN)))
    ...
    verify(seq) :+ Transaction(Amount(-1000), LocalDate.MIN)
  }
